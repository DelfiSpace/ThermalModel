...
% Ask for the day of the simulation
day = input('What is the day of the simulation? \n') ;

% Load the satellite properties
...
teta1 = 0:2*pi/(T_orbit*orbit):2*pi*orbit ;
...
for i=1:length(teta1)
   [x(i,2), x(i,3), x(i,1)] = HeatReceived_perso(day,beta,teta1(i),h,eta_X,0.3) ;
   x(i,4) = sum(x(i,:)) ;
   
   [xm(i,2), xm(i,3), xm(i,1)] = HeatReceived_perso(day,beta,teta1(i),h,eta_Xm,0.3) ;
   xm(i,4) = sum(xm(i,:)) ;
   
   [y(i,2), y(i,3), y(i,1)] = HeatReceived_perso(day,beta,teta1(i),h,eta_Y,0.3) ;
   y(i,4) = sum(y(i,:)) ;
   
   [ym(i,2), ym(i,3), ym(i,1)] = HeatReceived_perso(day,beta,teta1(i),h,eta_Ym,0.3) ;
   ym(i,4) = sum(ym(i,:)) ;
   
   [z(i,2), z(i,3), z(i,1)] = HeatReceived_perso(day,beta,teta1(i),h,eta_Z,0.3) ;
   z(i,4) = sum(z(i,:)) ;
   
   [zm(i,2), zm(i,3), zm(i,1)] = HeatReceived_perso(day,beta,teta1(i),h,eta_Zm,0.3) ;
   zm(i,4) = sum(zm(i,:)) ;
end
...
% Same matrices as in Stefano code
